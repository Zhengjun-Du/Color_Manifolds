...
class SOM:
    ...
    def _train(self, t):
        ...
        # update weight
        # distances from bmu to other nodes
        D = normVectors(self._positions - bmu_position) 
        # L = L0 exp (-t / lambda), L0:0.16
        L = self._learningRestraint(t) 
        # T = exp (-D^2 / 2 sigma^2),sigma = self._sigma0 * np.exp(-t / self._lmbd)
